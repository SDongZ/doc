...
/**
g_object_new 流程 http://blog.csdn.net/giesus/article/details/5891773

这个函数是个可变参数的函数, 第一个参数是需要创建的对象的类型, 
                            当使用 g_object_new 来创建对象的时候, 这个参数是必须的, 
                            同时它还要求这个函数所创建 的对象必须是GObject的子对象. 
在我们定义自己的对象时, 必须要在系统中注册自己的类型, 这里的系统指的是 glib 的系统, 即 glib 自己维护的一套数据结构.
*/
...
/**
https://developer.gnome.org/glib/stable/index.html  GLib 文档
#include <glib.h>
typedef             gchar;                     //typedef char   gchar;
struct              GError;                    //Error Reporting — a system for reporting errors
typedef struct _GOptionContext GOptionContext; //A GOptionContext struct defines which options are accepted by the commandline option parser. 
...
Types which correspond exactly to standard C types, but are included for completeness(完整性) - gchar, gint, gshort, glong, gfloat, gdouble.
...
GLib 为 C 语言编写的库和程序提供了核心应用程序组件。
它提供了 GNOME 中使用的核心对象系统， main 循环的实现以及操作字符串和常用数据结构的一整套工具函数

GLib is a cross-platform software utility library that began as part of the GTK+ project. 
However, before releasing version 2 of GTK+, 
the project's developers decided to separate non-GUI-specific code from the GTK+ platform, thus creating GLib as a separate product. 
GLib was released as a separate library so other developers, 
those who did not make use of the GUI-related portions of GTK+, 
could make use of the non-GUI portions of the library without the overhead of depending on the entire GUI library.

glibc is a core C runtime library. It provides things like printf(3) and fopen(3).  the GNU C Library, commonly known as glibc
...
struct spice_connection {
    ...
    SpiceSession     *session;   
#include "spice-types.h"  typedef struct _SpiceSession SpiceSession;  
struct _SpiceSession
{
    GObject parent;
    SpiceSessionPrivate *priv; //Do not add fields to this struct  见spice-session-priv.h 拥有很多字段
};
    SpiceGtkSession  *gtk_session;
    SpiceMainChannel *main;
    SpiceWindow     *wins[CHANNELID_MAX * MONITORID_MAX];
...
/**
Libintl is a library that provides native language support to programs. It is part of Gettext. Homepage
...
kill()系统调用会在权限允许的情况下向进程发送特定的信号，类似地，kill命令允许用户向进程发送信号。
                                                                raise(3)库函数可以将特定信号发送给当前进程。

像除数为零、断错误这些异常也会产生信号（这里分别是SIGFPE和SIGSEGV，默认都会导致进程终止和核心转储).
内核可以向进程发送信号以告知它一个事件发生了。例如当进程将数据写入一个已经被关闭的管道是将会收到SIGPIPE信号，默认情况下会使进程关闭。
*/
    signal(SIGINT, signal_handler);
    signal(SIGTERM, signal_handler);
    segv_handler = signal(SIGSEGV, signal_handler); SIGSEGV的符号常量在头文件signal.h中定义。通常，它是信号11。
...
GKeyFile lets you parse, edit or create files containing groups of key-value pairs, which we call key files for lack of a better name. 
...
The syntax of key files is described in detail in the Desktop Entry Specification, here is a quick summary: 
...
*/
	...
	/**
	On Windows this is the folder to use for local (as opposed to roaming) application data. 
	See documentation for CSIDL_LOCAL_APPDATA. 
	Note that on Windows it thus is the same as what g_get_user_config_dir() returns.
	*/
...
All options in a group share the same translation function. 
Libraries which need to parse commandline options are expected to provide a function for getting a GOptionGroup holding their options, 
which the application can then add to its GOptionContext.
...
*/
    gtk_init(&argc, &argv);
    context = g_option_context_new(_("- spice client test application"));  //Creates a new option context
    ...
    g_option_context_add_group(context, spice_get_option_group());  // spice-option.h spice-option.c
    g_option_context_set_main_group(context, spice_cmdline_get_option_group());  //#include "spice-cmdline.h" 指定 uri, host, port, password
